...
| 2026-07-12 | **WP 1.2** — Rule writes gated to the permissions admin (interim) until the WP 1.3 approval-gated editor lands. Rules seed via migration/superuser until then. | M1-R4: no rule silently changes behavior; an interim staff-writable policy would violate that before the approval workflow exists. |
| 2026-07-12 | **WP 1.2** — `task_provenance` immutability: `authenticated` gets **no UPDATE/DELETE grant** (belt) and no RLS policy for either (suspenders); even the permissions admin cannot modify it. Rule-version selection uses `effective_from <= ref_date` (latest such version) — rules apply as they stood on the trigger date. | M1-R14: permanent, immutable provenance is the parallel-run diff's instrument; retro-dated triggers must reproduce historical calculations. |
